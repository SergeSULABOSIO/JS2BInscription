...
public enum Nombre {
    ...
    //enum de calcul
    CALCULATE() {
                private String getValue(long value) throws Exception {
                    if (value == 0) {
                        return ZERO.label;
                    } else {
                ...
                public String getLettres(double montant, String monnaie) throws Exception {
                    double montantArr = UtilInscription.round(montant, 2);
                    int montantArrInt = (int) montantArr;

                    double montantArrIntApVirg = montant - montantArrInt;
                    montantArrIntApVirg = UtilInscription.round(montantArrIntApVirg, 2);
                    ...
                    String montantAvantVirgule = "" + getValue(montantArrInt);

                    String finale = "";
                    if(0<montantArrIntApVirg){
                        finale = montantAvantVirgule + " " +monnaie +", " + montantApreVirgule;
                    }
                    if(0==montantArrIntApVirg){
                        finale = montantAvantVirgule + " " +monnaie;
                    }
                    
                    return finale.toUpperCase();
                }

                private String getValue(double value, String separator) throws Exception {
                    if (value == 0) {
    ...
    protected String getLettres(double montant, String monnaie) throws Exception {
        throw new Exception("Vous devez appeller la méthode par l'énumération Chiffre.CALCULATE");
    }
